...
def generate_groupings(num_students,num_groups):
	...
	if num_students < num_groups:
		raise ValueError('number of groups must be less than or equal to number of students')

	# Dynamic programming method for generating groupings
	groups = [[x] for x in xrange(num_groups)]
	for i in xrange(1,num_students):
		groups = [ x + [c] for c in xrange(num_groups) for x in groups ]
	return groups
